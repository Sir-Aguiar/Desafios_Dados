...
class Validador:
    ...
    @staticmethod
    def _formato_data_valido(valor, formato):
        """Retorna True se o valor respeita exatamente o formato informado."""
        if valor is None or (isinstance(valor, float) and pd.isna(valor)):
            return False

        texto = str(valor).strip()

        # Se o texto for NaN, retorna False
        if not texto or texto.lower() == "nan":
            return False

        try:
            # Tenta converter o texto para o formato informado
            datetime.strptime(texto, formato)
            return True
        except (ValueError, TypeError):
            # Se der ValueError ou TypeError, retorna False
            return False
    ...
    def validar_catalogo(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            if cid in ids_vistos:
                self._registrar_motivo(
                    "catalogo", cid, "conteudo_id duplicado", "duplicados"
                )
                continue
            ids_vistos.add(cid)
            ...
            if faltando:
                self._registrar_motivo(
                    "catalogo", cid, "campos ausentes: " + str(faltando), "incompletos"
                )
                continue
            ...
            try:
                ...
                motivos.append(
                    "carga_horaria nao-numerica: " + str(row["carga_horaria_min"])
                )
            if not self._formato_data_valido(row["data_publicacao"], "%Y-%m-%d"):
                motivos.append(
                    "data_publicacao em formato invalido: "
    ...
    def validar_interacoes(self, df, ids_conteudos_validos, ids_usuarios_validos):
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                motivos.append(
                    "tempo_consumido nao-numerico: " + str(row["tempo_consumido"])
                )
            try:
                ...
                motivos.append(
                    "percentual nao-numerico: " + str(row["percentual_conclusao"])
                )
            ...
            if not self._formato_data_valido(row["data_hora"], "%Y-%m-%dT%H:%M:%S"):
                motivos.append(
                    "data_hora em formato invalido: "
                    + str(row["data_hora"])
        ...
        self.logger.info(
            f"  Interacoes: {r['validos']} validos, {r['invalidos']} invalidos, {r['incompletos']} incompletos, {r['duplicados']} duplicados"
        )

    # ------------------------------------------------------------------
    # Validar os comentarios
    # ------------------------------------------------------------------
    def validar_comentarios(self, lista, ids_conteudos_validos, ids_usuarios_validos):
        ...
        for coment in lista:
            ...
            if not self._formato_data_valido(coment["data"], "%Y-%m-%d"):
                motivos.append(
